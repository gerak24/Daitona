...
window.t_forms__lang = (window.navigator.userLanguage || window.navigator.language).toUpperCase().slice(0, 2), window.scriptSysPayment = {}, window.handlerSysPayment = {}, window.isInitEventsZB = {}, window.isInitEventsCustomMask = {}, window.initForms = {}, window.tildaForm = {
    ...
    if (isST100 || isST105 || isExistsPay) {
        ...
        if (!document.head.querySelector('script[src*="' + fileName + '"]')) (script = document.createElement("script")).type = "text/javascript", script.src = "https://static.tildacdn.com/js/" + fileName + ".min.js", script.onerror = function () {
    ...
    if (!0 !== window.isInitEventsCustomMask) {
        ...
        script.type = "text/javascript", script.src = "https://static.tildacdn.com/js/tilda-forms-custommask-1.0.min.js", document.head.appendChild(script), window.isInitEventsCustomMask = !0
    }
    ...
    for (var inputs = form.querySelectorAll(".js-tilda-rule"), arrError = [], isEmptyValue = !0, i = 0; i < inputs.length; i++) {
        var input = inputs[i], isReq = !!parseInt(input.getAttribute("data-tilda-req") || 0, 10),
    ...
    if ("y" === dataFormCart) {
        var isMinOrderSetted = void 0 !== window.tcart_minorder && window.tcart_minorder > 0,
            isMinQuantitySetted = void 0 !== window.tcart_mincntorder && window.tcart_mincntorder > 0, objError,
            objError;
        if (isMinOrderSetted) if (window.tcart.prodamount < window.tcart_minorder) (objError = {
            obj: {},
            type: []
    ...
    inputBoxClassName || (inputBoxClassName = ".blockinput");
    for (var inputGroup = "", isErrorBox = !1, isShowErrors = !0, errorItem = "", errorInputs = "", strError = "", i = 0; i < arrErrors.length; i++) if (arrErrors[i] && arrErrors[i].obj) {
        if (0 === i && "none" === arrErrors[i].obj) {
        ...
        for (var j = 0; j < arrErrors[i].type.length; j++) {
            ...
            if (strError = "", (errorItem = form.querySelectorAll(".js-rule-error-" + error)).length) for (var k = 0; k < errorItem.length; k++) errorItem[k].getAttribute("data-rule-filled") ? errorItem[k].style.display = "block" : (errorItem[k].textContent && errorItem[k].innerText || !t_forms__getMsg(error) ? strError = errorItem[0].textContent || errorItem[0].innerText : errorItem[k].innerHTML = t_forms__getMsg(error), errorItem[k].style.display = "block"); else if (t_forms__getMsg(error) && (errorItem = form.querySelectorAll(".js-rule-error-all")).length) for (var k = 0; k < errorItem.length; k++) errorItem[k].innerHTML = t_forms__getMsg(error), errorItem[k].style.display = "block";
            ...
            if (btnSubmit.tildaSendingStatus && (btnStatus = btnSubmit.tildaSendingStatus), !(btnStatus && btnStatus >= 1 || t_hasClass(btnSubmit, "t706__submit_disable"))) {
                ...
                if (!form.querySelectorAll(".js-formaction-services").length && 1 !== formType && !formKey) {
                    var msgContainers = t_forms__getErrorContainers(form, ""), errorBoxes = msgContainers.errorBoxes,
                        allError = msgContainers.allError;
                    return Array.prototype.forEach.call(allError, (function (error) {
                        error.innerHTML = "На время проведения технических работ, форма недоступна, воспользуйтесь контактами ниже", error.style.display = "block"
...
function t_forms__onSuccess(form) {
    ...
    var inputsboxHiddenClassName = "t" + recType + "__inputsbox_hidden", arrNotScroll = [702, 708, 862, 945, 1014],
        isScroll = !0;
...
function t_serializeArray(form) {
    for (var arr = [], elements = form.querySelectorAll("input, textarea, button, select"), i = 0; i < elements.length; i++) if (!(!elements[i].name || elements[i].disabled || ["file", "reset", "submit", "button"].indexOf(elements[i].type) > -1)) if ("select-multiple" !== elements[i].type) ["checkbox", "radio"].indexOf(elements[i].type) > -1 && !elements[i].checked || arr.push({
